...
建議在電腦上下載、安裝及使用。點下方「下載 ZIP」即可，不需要尋找程式碼或整個儲存庫的下載按鈕。
...
減少 AI 常見的修辭套句，用直接清楚的中文表達。保留原意、事實、語氣與「預計／待確認」等狀態。

這份 Skills 已整理好寫作流程與檢查重點，你可以重複使用，不用每次從頭教 AI 怎麼協助準備藝文補助企劃。
...
1. 準備當次簡章、官方範本及作品資料；海外交流另準備邀請文件、行程與出國人員資料。
2. 啟用技能，告訴 AI 你要申請什麼，以及想採用哪種寫法。
3. 依整理結果逐步補資料、寫作、修訂與檢查。
4. 需要 Word 時，請 AI 運用目前環境的檔案功能產出可編輯的 `.docx`，下載後再確認內容與版面。

你可以這樣開始：

> 請使用企劃寫作與中文修飾 Skills。這是本次簡章與我的作品資料。我想先列出段落骨架，自己擴寫。請依官方欄位整理每段要回答什麼、可以使用哪些已知素材，缺少的資料標為待補。

想先看草稿時：

> 請依這份簡章和我提供的資料，先寫內文，我來修改。尚未確認的人員、行程、合作或金額請標為待補，不要自行補造。
